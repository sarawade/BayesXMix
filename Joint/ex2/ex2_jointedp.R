...
setwd("/Users/swade/Documents/GitHub/BayesXMix/Joint/ex2")
...
k0y = 5 # prior guess on the number of clusters, divide the SD of the linear regression residuals by this factor to encourage smaller variance within cluster
b_y=sum(lmfit$residuals^2)/(n-p-1)/k0y
...
png("ex2_jedp_clus.png",width = 500, height = 450)
ggplot() +
...
png("ex2_jedp_pred.png",width = 500, height = 450)
ggplot() +
...
png("ex2_jedp_fpred.png",width = 500, height = 450)
cols = rainbow(6)
ggplot() +
...
png("ex2_jedp_dens_heat",width = 500, height = 450)
f_pred = data.frame(x = rep(x_new, each = m2), y = rep(y_grid,n_new), density= c(output_pred$f_pred))
ggplot(f_pred) +
...
coeff = max(l1_dist)
png("ex2_jedp_l1dist",width = 500, height = 500)
ggplot() +
