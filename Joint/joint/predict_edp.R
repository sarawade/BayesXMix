...
predict_edp=function(S,m,m2,p,x_new, y_grid, mu_theta, C, a_y, b_y, mu_0, c_x, a_x, b_x, k_y, k_x, config_y, config_x, alpha_x, alpha_y, phi_y, sigma_y, mu_x, sigma_x ){
...
for(s in 1:S){
	...
	weight_mat_s[,1]=alpha_y[s]/(alpha_y[s]+n)*marg_x
	f_pred=f_pred+marg_y%*%diag(weight_mat_s[,1])
	phi_s=matrix(phi_y[[s]],p+1,k_y[s])

	for(j in 1:k_y[s]){
		nj_s=sum(config_y[s,]==j)
		weight_mat_s[,j+1]=nj_s/(alpha_y[s]+n)*marg_x*alpha_x[[s]][j]/(alpha_x[[s]][j]+nj_s)
		for(h in 1:k_x[[s]][j]){
			...
			weight_mat_s[,j+1]=weight_mat_s[,j+1]+nj_s/(alpha_y[s]+n)*pred_x_new_jhs*njh_s/(alpha_x[[s]][j]+nj_s)
		}
		f_pred=f_pred+dnorm(matrix(y_grid,nrow=m2,ncol=m),t(matrix(X_new%*%phi_s[,j],nrow=m,ncol=m2)),sigma_y[[s]][j]^.5 )%*%diag(weight_mat_s[,j+1])
	}
	#calculate prediction
	y_pred_s=X_new%*%cbind(mu_theta,phi_s)
