...
setwd("/Users/swade/Documents/GitHub/BayesXMix/Joint/ex3")
...
#Heat map of mean
png("ex3_jdp_pred_heat.png",width = 500, height = 450)
df = data.frame(x1 =x_new[,1],x2=x_new[,2],y = output_cred_pred$y_pred)
ggplot(df) +
...
png("ex3_jdp_pred.png",width = 500, height = 450)
xnew2 = 1.5
ggplot() +
  geom_point(aes(x = x_new[x_new[,2]==xnew2,1], y = output_cred_pred$y_pred[x_new[,2]==xnew2]), col = "black") +
  geom_point(aes(x = x_new[x_new[,2]==xnew2,1], y = m_true_new[x_new[,2]==xnew2]), col = "red") +
  geom_ribbon(aes(x = x_new[x_new[,2]==xnew2,1],ymin=output_cred_pred$l_pred[x_new[,2]==xnew2], ymax=output_cred_pred$u_pred[x_new[,2]==xnew2]), alpha=0.2) +
  theme_bw() +
  labs( x = "x_1", y = "y")
dev.off()
...
png("ex3_jdp_fpred.png",width = 500, height = 450)
cols = rainbow(6)
ggplot() +
...
#Heat map of mean
png("ex3_jdp_l1_heat.png",width = 500, height = 450)
