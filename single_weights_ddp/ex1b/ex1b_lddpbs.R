...
for(i in 1:m2){
  f_true_new[i, ] <- dsn(y_grid[i] - m_true_new, xi=xi_new, 
                         omega = omega, alpha = alpha_new)
}
...
cols <- rainbow(6)
ggplot() +
  geom_line(aes(x = y_grid, y = dpred_lddp_bs_0_m[, inds[1]]), col = cols[1]) +
  geom_line(aes(x = y_grid, y = f_true_new[, inds[1]]), 
            col = cols[1], linetype = "dashed") +
  geom_ribbon(aes(x = y_grid, ymin = dpred_lddp_bs_0_l[, inds[1]], ymax = dpred_lddp_bs_0_h[, inds[1]]), 
              alpha = 0.2, fill = cols[1]) +
 geom_line(aes(x = y_grid, y = dpred_lddp_bs_0_m[, inds[3]]), col = cols[3]) +
  geom_line(aes(x = y_grid, y = f_true_new[, inds[3]]), 
            col = cols[3], linetype = "dashed") +
  geom_ribbon(aes(x = y_grid, ymin = dpred_lddp_bs_0_l[, inds[3]], ymax = dpred_lddp_bs_0_h[, inds[3]]), 
              alpha = 0.2, fill = cols[3]) +
  geom_line(aes(x = y_grid, y = dpred_lddp_bs_0_m[, inds[5]]), col = cols[5]) +
  geom_line(aes(x = y_grid, y = f_true_new[, inds[5]]), 
            col = cols[5], linetype = "dashed") +
  geom_ribbon(aes(x = y_grid, ymin = dpred_lddp_bs_0_l[, inds[5]], ymax = dpred_lddp_bs_0_h[, inds[5]]), 
              alpha = 0.2, fill = cols[5]) +
      alpha = 0.2, fill = cols[6]) +
  theme_bw() +
  ylim(0, 8.75) + 
  labs( x = "y", y = "Density") 
