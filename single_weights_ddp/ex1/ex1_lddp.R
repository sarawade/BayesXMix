...
# MCMC realisations predictive regression function
...
# Plot predictive regression function 
ggplot() +
...
cols <- rainbow(6)
ggplot() +
  geom_line(aes(x = y_grid, y = dpred_lddp_m[, 1]), col = cols[1]) +
  geom_line(aes(x = y_grid, y = f_true_new[, 1]), 
            col = cols[1],linetype = "dashed") +
  geom_ribbon(aes(x = y_grid, ymin = dpred_lddp_l[, 1], ymax = dpred_lddp_h[, 1]), 
              alpha = 0.2, fill = cols[1]) +
  geom_line(aes(x = y_grid, y = dpred_lddp_m[, 201]), col = cols[3]) +
  geom_line(aes(x = y_grid, y = f_true_new[, 201]), 
            col = cols[3],linetype = "dashed") +
  geom_ribbon(aes(x = y_grid, ymin = dpred_lddp_l[, 201], ymax = dpred_lddp_h[, 201]), 
              alpha = 0.2, fill = cols[3]) +
  geom_line(aes(x = y_grid, y = dpred_lddp_m[, 401]), col = cols[5]) +
  geom_line(aes(x = y_grid, y = f_true_new[, 401]), 
            col = cols[5],linetype = "dashed") +
  geom_ribbon(aes(x = y_grid, ymin = dpred_lddp_l[, 401], ymax = dpred_lddp_h[, 401]),
