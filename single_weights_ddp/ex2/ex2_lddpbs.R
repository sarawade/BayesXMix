...
waic0$WAIC; lpml0$lpml

knots1 <- quantile(x[, 1], 0.5)
X1 <- cbind(rep(1, n),
...
knots2 <- quantile(x[, 1], c(0.33, 0.66))
X2 <- cbind(rep(1, n),
...
knots3 <- quantile(x[, 1], c(0.25, 0.5, 0.75))
X3 <- cbind(rep(1, n),
...
knots4 <- quantile(x[, 1], c(0.2, 0.4, 0.6, 0.8))
X4 <- cbind(rep(1, n),
...
# Model with 5 knots is preferred and so the remaining is computed for this model
# Estimated clustering and plot
psm_lddpbs <- comp.psm(fit_lddp_bs_5$z)
output_vi_lddpbs <- minVI(psm_lddpbs, fit_lddp_bs_5$z)
...
for(l in 1:mcmc$nsave){
  for(j in 1:n_new){  
    m_pred_lddp_bs[j, l] <- sum(fit_lddp_bs_5$P[l,]*X5_predict[j,]%*%t(fit_lddp_bs_5$Beta[l,,]))  
  }
}
...
ggplot() +
  geom_line(aes(x = x_new[,1], y = m_pred_lddp_bs_m), col = "black") +
  geom_line(aes(x = x_new[,1], y = m_true_new), col = "red") +
  geom_ribbon(aes(x = x_new[,1], ymin = m_pred_lddp_bs_l, ymax = m_pred_lddp_bs_h),
              alpha = 0.2) +
  theme_bw() +
  labs( x = "x_1", y = "y") 

#empirical l2 prediction error
l2_err_lddp_bs <- sum(((m_true_new - m_pred_lddp_bs_m)^2)/n_new)^.5
l2_err_lddp_bs

#empirical coverage for prediction
ec_pred_lddp_bs <- sum((m_true_new >= m_pred_lddp_bs_l)&(m_true_new <= m_pred_lddp_bs_h))/n_new
ec_pred_lddp_bs

# credible interval length for prediction
ci_length_lddp_bs <- m_pred_lddp_bs_h - m_pred_lddp_bs_l
mean(ci_length_lddp_bs)

# Predictive density function and plot
index <- seq(1, mcmc$nsave, by = 3)
dpred_lddpbs <- array(0,c(length(index), m2, n_new))

for(k in 1:length(index)){
  mu <- tcrossprod(X5_predict, fit_lddp_bs_5$Beta[(k*3) - 2, , ])
  
  for(l in 1:n_new) {
    aux <- norMix(mu = c(mu[l, ]), sigma = sqrt(fit_lddp_bs_5$Sigma2[(k*3) - 2,]), w = fit_lddp_bs_5$P[(k*3) - 2,])
...
f_pred_lddpbs <- data.frame(x = rep(x_new, each = m2), 
                          y = rep(y_grid, n_new), 
                          density = c(dpred_lddpbs_m))

ggplot(f_pred_lddpbs) +
