...
# General MCMC setting for all knots' configurations
...
psm_lddpbs <- comp.psm(fit_lddp_bs_2$z)
output_vi_lddpbs <- minVI(psm_lddpbs, fit_lddp_bs_2$z)
...
m_pred_lddp_bs_2 <- matrix(0, nrow = n_new, ncol = mcmc$nsave)
for(l in 1:mcmc$nsave){
  for(j in 1:n_new){  
   m_pred_lddp_bs_2[j, l] <- sum(fit_lddp_bs_2$P[l,]*X2_predict[j,]%*%t(fit_lddp_bs_2$Beta[l,,]))  
  }
}  

m_pred_lddp_bs_2_m <- apply(m_pred_lddp_bs_2, 1, mean)
m_pred_lddp_bs_2_l <- apply(m_pred_lddp_bs_2, 1, quantile, prob = 0.025)
m_pred_lddp_bs_2_h <- apply(m_pred_lddp_bs_2, 1, quantile, prob = 0.975)

ggplot() +
  geom_line(aes(x = x_new[ , 1], y = m_pred_lddpbs_m), col = "black") +
  geom_line(aes(x = x_new[ , 1], y = m_true_new), col = "red") +
  geom_ribbon(aes(x = x_new[ , 1], ymin = m_pred_lddpbs_l, ymax = m_pred_lddpbs_h),
              alpha = 0.2) +
  theme_bw() +
  labs( x = "x_1", y = "y")+
  ylim(1.7, 5.1)
...
l2_err_lddpbs <- sum(((m_true_new - m_pred_lddp_bs_2_m)^2)/n_new)^.5
l2_err_lddpbs

# Empirical coverage for prediction
ec_pred_lddpbs <- sum((m_true_new >= m_pred_lddp_bs_2_l)&(m_true_new <= m_pred_lddp_bs_2_h))/n_new
ec_pred_lddpbs

# Credible interval length for prediction
ci_length_lddpbs <- m_pred_lddp_bs_2_h - m_pred_lddp_bs_2_l
mean(ci_length_lddpbs)

# Predictive densities and plot
dpred_lddp_bs_2 <- array(0, c(mcmc$nsave, m2, n_new))

for(k in 1:mcmc$nsave){
  mu <- tcrossprod(X2_predict, fit_lddp_bs_2$Beta[k, , ])
  
  for(l in 1:n_new) {
    aux <- norMix(mu = c(mu[l,]), sigma = sqrt(fit_lddp_bs_2$Sigma2[k,]), w = fit_lddp_bs_2$P[k,])
    dpred_lddp_bs_2[k, , l] <- dnorMix(y_grid, aux)
  }
}

dpred_lddp_bs_2_m <- apply(dpred_lddp_bs_2, c(2,3), mean)
dpred_lddp_bs_2_l <- apply(dpred_lddp_bs_2, c(2,3), quantile, prob = 0.025)
dpred_lddp_bs_2_h <- apply(dpred_lddp_bs_2, c(2,3), quantile, prob = 0.975)

# Estimated l1 distance for density
l1_dist_lddpbs <- colSums(abs(f_true_new - dpred_lddp_bs_2_m))*(y_grid[2]-y_grid[1])
mean(l1_dist_lddpbs)
...
cols <- rainbow(6)
ggplot() +
  geom_line(aes(x = y_grid, y = dpred_lddp_bs_2_m[, inds[1]]), col = cols[1]) +
  geom_line(aes(x = y_grid, y = f_true_new[, inds[1]]), 
            col = cols[1], linetype = "dashed") +
  geom_ribbon(aes(x = y_grid, ymin = dpred_lddp_bs_2_l[, inds[1]], ymax = dpred_lddp_bs_2_h[, inds[1]]), 
              alpha = 0.2, fill = cols[1]) +
  geom_line(aes(x = y_grid, y = dpred_lddp_bs_2_m[, inds[3]]), col = cols[3]) +
  geom_line(aes(x = y_grid, y = f_true_new[, inds[3]]), 
            col = cols[3], linetype = "dashed") +
  geom_ribbon(aes(x = y_grid, ymin = dpred_lddp_bs_2_l[, inds[3]], ymax = dpred_lddp_bs_2_h[, inds[3]]), 
              alpha = 0.2, fill = cols[3]) +
  geom_line(aes(x = y_grid, y = dpred_lddp_bs_2_m[, inds[5]]), col = cols[5]) +
  geom_line(aes(x = y_grid, y = f_true_new[, inds[5]]), 
            col = cols[5], linetype = "dashed") +
  geom_ribbon(aes(x = y_grid, ymin = dpred_lddp_bs_2_l[, inds[5]], ymax = dpred_lddp_bs_2_h[, inds[5]]), 
