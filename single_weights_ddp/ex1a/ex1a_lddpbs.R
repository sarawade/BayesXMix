...
### general MCMC setting for all knots's configurations
...
# Model with one knot is preferred and so the remainder calculations are done for this model
# Estimated clustering and plot
psm_lddpbs_1 <- comp.psm(fit_lddp_bs_1$z)
output_vi_lddpbs_1 <- minVI(psm_lddpbs_1, fit_lddp_bs_1$z)

ggplot() +
  geom_point(aes(x = x[, 1], y = y, color = as.factor(output_vi_lddpbs_1$cl))) +
  theme_bw() +
  labs( x = "x_1", y = "y", color = "Cluster") +
  geom_line(aes(x = x[, 1], y = m_true), col ="red")

# Predictive regression function and plot
m_pred_lddp_bs_1 <- matrix(0, nrow = n_new, ncol = mcmc$nsave)
for(l in 1:mcmc$nsave){
  for(j in 1:n_new){ 
  m_pred_lddp_bs_1[j, l] <- sum(fit_lddp_bs_1$P[l,]*X1_predict[j,]%*%t(fit_lddp_bs_1$Beta[l,,]))  
  }
}

m_pred_lddp_bs_1_m <- apply(m_pred_lddp_bs_1, 1, mean)
m_pred_lddp_bs_1_l <- apply(m_pred_lddp_bs_1, 1, quantile, prob = 0.025)
m_pred_lddp_bs_1_h <- apply(m_pred_lddp_bs_1, 1, quantile, prob = 0.975)

ggplot() +
  geom_line(aes(x = x_new[ , 1], y = m_pred_lddp_bs_1_m), col = "black") +
  geom_line(aes(x = x_new[ , 1], y = m_true_new), col = "red") +
  geom_ribbon(aes(x = x_new[ , 1], ymin = m_pred_lddp_bs_1_l, ymax = m_pred_lddp_bs_1_h),
              alpha = 0.2) +
  theme_bw() +
  labs( x = "x_1", y = "y")+
  ylim(1.7, 5.1)
...
l2_err_lddpbs <- sum(((m_true_new - m_pred_lddp_bs_1_m)^2)/n_new)^.5
l2_err_lddpbs

# Empirical coverage for prediction
ec_pred_lddpbs <- sum((m_true_new >= m_pred_lddp_bs_1_l)&(m_true_new <= m_pred_lddp_bs_1_h))/n_new
ec_pred_lddpbs
...
for(k in 1:mcmc$nsave){
  mu <- tcrossprod(X1_predict, fit_lddp_bs_1$Beta[k, , ])
  
  for(l in 1:n_new) {
    aux <- norMix(mu = c(mu[l,]), sigma = sqrt(fit_lddp_bs_1$Sigma2[k,]), w = fit_lddp_bs_1$P[k,])
    dpred_lddp_bs_1[k, , l] <- dnorMix(y_grid, aux)
  }
}

dpred_lddp_bs_1_m <- apply(dpred_lddp_bs_1, c(2,3), mean)
dpred_lddp_bs_1_l <- apply(dpred_lddp_bs_1, c(2,3), quantile, prob = 0.025)
dpred_lddp_bs_1_h <- apply(dpred_lddp_bs_1, c(2,3), quantile, prob = 0.975)

# Estimated l1 distance for density
l1_dist_lddpbs <- colSums(abs(f_true_new - dpred_lddp_bs_1_m))*(y_grid[2]-y_grid[1])
