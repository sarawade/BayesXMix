...
setwd("/Users/swade/Documents/GitHub/BayesXMix/NormalizedWeights/ex2")
...
config=as.matrix(read.csv("ex2_config_nwreg.csv", header = FALSE))
S = dim(config)[1]
k=rep(0,S)
for(s in 1:S){
...
png("ex2_nw_clus.png",width = 500, height = 450)
ggplot() +
...
### PREDICTION
pred=as.matrix(read.csv("ex2_pred_nwreg.csv", header = FALSE))
fpred=as.matrix(read.csv("ex2_fpred_nwreg.csv", header = FALSE))
lfpred=as.matrix(read.csv("ex2_lfpred_nwreg.csv", header = FALSE))
ufpred=as.matrix(read.csv("ex2_ufpred_nwreg.csv", header = FALSE))

###Plot Prediction
#with credible intervals but without data
png("ex2_nw_pred.png",width = 500, height = 450)
ggplot() +
...
png("ex2_nw_fpred.png",width = 500, height = 450)
cols = rainbow(6)
ggplot() +
...
png("ex2_nw_dens_heat",width = 500, height = 450)
f_pred = data.frame(x = rep(x_new, each = m2), y = rep(y_grid,n_new), density= c(t(fpred)))
ggplot(f_pred) +
...
coeff = max(l1_dist)
png("ex2_nw_l1dist",width = 500, height = 500)
ggplot() +
