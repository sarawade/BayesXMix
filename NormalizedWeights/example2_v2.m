...
Y_fpred_true(x_new>2,:)=normpdf(repmat(Y_grid',sum(x_new>2),1),repmat(m_pred_true(x_new>2),1,ny),0.01);
Y_fpred_true(x_new<=2,:)=.5*normpdf(repmat(Y_grid',sum(x_new<=2),1),repmat(m_pred_true(x_new<=2),1,ny),0.01)+.5*normpdf(repmat(Y_grid',sum(x_new<=2),1),repmat(m_pred_true(x_new<=2),1,ny),0.05);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%mcmc=[2000,1,2000];
save 'example2_nwreg_v2'

csvwrite('ex1_pred_nwreg.csv',[Y_pred,Y_pred_CI(:,1:2)]);
csvwrite('ex1_fpred_nwreg.csv',Y_fpred);
csvwrite('ex1_lfpred_nwreg.csv',l_fpred);
csvwrite('ex1_ufpred_nwreg.csv',u_fpred);
csvwrite('ex1_config_nwreg.csv',d);
