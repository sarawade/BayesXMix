%% EXAMPLE %%
% Supplementary Example from Wade and Inacio (2024): data generated from a
% circle
...
M=csvread('ex_circle/extrain_circle.csv',1,1);
Y=M(:,2);
x=M(:,1);
% Sample size
n=size(Y,1);
...
x_new=csvread('ex_circle/extest_circle.csv',1,1);
n_new=size(x_new,1);
% For simulated data, we can calculate the true predictive mean for
% comparison with estimates
m_pred_true=zeros(n_new,1);
...
% comparison with estimates
Y_fpred_true = csvread('ex_circle/extest_circle_dens.csv',1,1);
...
save 'ex_circle/example_nwreg_circle'

csvwrite('ex_circle/ex_pred_nwreg_circle.csv',[Y_pred,Y_pred_CI(:,1:2)]);
csvwrite('ex_circle/ex_fpred_nwreg_circle.csv',Y_fpred);
csvwrite('ex_circle/ex_config_nwreg_circle.csv',d);
csvwrite('ex_circle/ex_lfpred_nwreg_circle.csv',l_fpred);
csvwrite('ex_circle/ex_ufpred_nwreg_circle.csv',u_fpred);
