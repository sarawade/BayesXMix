%% EXAMPLE %%
% Example 1a from Wade and Inacio (2024): extending example 1 with student
% t errors
...
M=csvread('ex1a/ex1train_terrors.csv',1,1);
Y=M(:,3);
x=M(:,1:2);
...
x_new=csvread('ex1a/ex1test_terrors.csv',1,1);
n_new=size(x_new,1);
...
% comparison with estimates
Y_fpred_true = csvread('ex1a/ex1test_terrors_dens.csv',1,1);
...
beta0=[4.115, -0.22, 0.0746]'; 
% sigma^2*iC is the variance for beta|sigma^2
iC=[0.15319555, -0.0264498111, -0.0120321522;-0.02644981,  0.0076936663, -0.0002818886; -0.01203215, -0.0002818886,  0.0119811259];
...
%% SAVE WORKSPACE %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
save 'ex1a/example1_nwreg_terrors'

csvwrite('ex1a/ex1_pred_nwreg_terrors.csv',[Y_pred,Y_pred_CI(:,1:2)]);
csvwrite('ex1a/ex1_fpred_nwreg_terrors.csv',Y_fpred);
csvwrite('ex1a/ex1_config_nwreg_terrors.csv',d);
csvwrite('ex1a/ex1_lfpred_nwreg_terrors.csv',l_fpred);
csvwrite('ex1a/ex1_ufpred_nwreg_terrors.csv',u_fpred);
