...
setwd("/Users/swade/Documents/GitHub/BayesXMix/NormalizedWeights/ex1c")
...
config=as.matrix(read.csv("ex1_config_nwreg_mmerrors.csv", header = FALSE))
S = dim(config)[1]
k=rep(0,S)
for(s in 1:S){
...
png("ex1_nw_clus_mmerrors.png",width = 500, height = 450)
ggplot() +
...
### PREDICTION
pred=as.matrix(read.csv("ex1_pred_nwreg_mmerrors.csv", header = FALSE))
fpred=as.matrix(read.csv("ex1_fpred_nwreg_mmerrors.csv", header = FALSE))
lfpred=as.matrix(read.csv("ex1_lfpred_nwreg_mmerrors.csv", header = FALSE))
ufpred=as.matrix(read.csv("ex1_ufpred_nwreg_mmerrors.csv", header = FALSE))

###Plot Prediction
#with credible intervals but without data
png("ex1_nw_pred_mmerrors.png",width = 500, height = 450)
ggplot() +
...
png("ex1_nw_fpred_mmerrors.png",width = 500, height = 450)
cols = rainbow(6)
ggplot() +
